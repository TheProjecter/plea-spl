	...
	public boolean handleCommand(Command c) {
		...
		/** Case: Save a copy in a new album */
			...
			try {
				...
				System.out.println( "<* PhotoViewController.handleCommand() *> init *Save Item*");
				...
				{
					try {
						System.out.println( "<* PhotoViewController.handleCommand() *> imageName: "+imageName);
				...
				System.out.println( "<* PhotoViewController.handleCommand() *> photoname: "+photoname+" | albumname: "+albumname);
				...
				// #if includeCopyMedia && (includeSmsFeature || capturePhoto)
				/* [NC] Added in scenario 06 and changed in scenario 8*/
				if ((imgByte==null) || (imgByte.length == 0))
				//#endif
				...
				/* [NC] Added in scenario 06 */
				{
					imageData.setMediaLabel(photoname);
